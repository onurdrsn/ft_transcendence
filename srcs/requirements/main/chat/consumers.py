...
import json
import asyncio
from chat.models import RoomChat, PrivateRoom, UnreadChat
...
class ChatConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def receive_json(self, content, byte_code=None):
        """
        ...
        try:
            if command == "join":
                ...
                if len(content["message"].lstrip()) == 0:
                    raise ClientError(422, _("You can't send an empty message."))
                ...
                if payload is not None:
                    ...
                    raise ClientError(204, _("Something went wrong retrieving the chatroom messages."))
                ...
                if payload is not None:
                    ...
                    raise ClientError(204, _("Something went wrong retrieving the other users account details."))
                await self.display_progress_bar(False)
    ...
    async def send_room(self, room_id, message):
        ...
        if self.room_id is not None:
            if str(room_id) != str(self.room_id):
                ...
                raise ClientError(_("ROOM_ACCESS_DENIED"), _("Room access denied"))
        else:
...
def get_room_or_error(room_id, user):
    ...
    try:
        ...
        raise ClientError(_("ROOM_INVALID"), _("Invalid room."))

    # Is this user allowed in the room? (must be user1 or user2)
    if user != room.user1 and user != room.user2:
        raise ClientError(_("ROOM_ACCESS_DENIED"), _("You do not have permission to join this room."))

    # Are the users in this room friends?
    friend_list = Friend.objects.get(user=user).friends.all()
    if room.user1 not in friend_list:
        if room.user2 not in friend_list:
            raise ClientError(_("ROOM_ACCESS_DENIED"), _("You must be friends to chat."))
    return room
...
def get_user_info(room, user):
    ...
    try:
        ...
        raise ClientError(_("DATA_ERROR"), _("Unable to get that users information."))


@database_sync_to_async
def create_room_chat_message(room, user, message):
    return RoomChat.objects.create(user=user, room=room, content=message)


@database_sync_to_async
def get_room_chat_messages(room, page_number):
    try:
        qs = RoomChat.objects.by_room(room)
        p = Paginator(qs, DEFAULT_ROOM_PAGE_SIZE)
...
def append_unread_msg_if_not_connected(room, user, connected_users, message):
    if user not in connected_users:
        try:
            unread_msgs = UnreadChat.objects.get(room=room, user=user)
            unread_msgs.most_recent_message = message
            unread_msgs.count += 1
            unread_msgs.save()
        except UnreadChat.DoesNotExist:
            UnreadChat(room=room, user=user, count=1).save()
            pass
    return
...
def on_user_connected(room, user):
    ...
    if user in connected_users:
        try:
            ...
            UnreadChat(room=room, user=user).save()
            pass
    return
